# Operational Management: Chef Recipe Execution and Automation

# Overview

The purpose of this series of AWS CloudFormation templates is to setup a scheduled multi-account and multi-Region (MAMR) Chef recipe execution operation using Amazon EventBridge rules, AWS Lambda, and AWS Systems Manager Automation. Additionally, Systems Manager Inventory and Chef RunInspec checks are enabled using a State Manager association. The Chef execution, software inventory, and compliance data gathered can then be queried and reported on using Amazon Athena or Amazon QuickSight.
